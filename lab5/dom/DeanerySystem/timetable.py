...
class TimetableWithoutBreaks(BasicTimetable):
    ...
    def __str__(self):
        ...
        out += "\n" + "*"*(8*20+7) + "\n"
        start = Term.rebuildFromMinutes(480)
        end = Term.rebuildFromMinutes(570)
        for i in range(0,8):
            time = str(start)[:-5] + "-" + str(end)[:-5]
            out += TimetableWithoutBreaks.align(time)
            for j in range(0,7):
                lesson = self.get(Term.rebuildFromMinutes(int(start)+1440*j))
                if(lesson == None):
                    out += "*" + " "*20 
                else:
...
class TimetableWithBreaks(BasicTimetable):
    def __init__(self, breaks: List[Break]):
        ...
        while(x <= 1170):
            ...
            if(break1 != None):
                ...
                self.validTerms.append(Term.rebuildFromMinutes(x))
                x += 90
    
    def busy(self, term: Term) -> bool:
        termMon = Term.rebuildFromMinutes(int(term) % 1440)
    ...
    def transferrer(self, lesson, pos):
        newIndex = -7
        termMon = Term.rebuildFromMinutes(int(lesson.term) % 1440)
    ...
    def __str__(self):
        out = " "*20
        for i in range(0,7):
            out += "*" + TimetableWithBreaks.align(Day(i).readable())
        out += "\n" + "*"*(8*20+7) + "\n"
        start = Term.rebuildFromMinutes(480)
        end = Term.rebuildFromMinutes(570)
        ...
        for i in range(len(self.validTerms)):
            time = str(start)[:-5] + "-" + str(end)[:-5]
            out += TimetableWithBreaks.align(time)
            break1 = None
            ...
            for j in range(0,7):
                ...
                if(lesson == None):
                    ...
                    out += "*" + TimetableWithBreaks.align(lesson.name)
            ...
            if(break1 == None):
                ...
                break2 = Break.rebuildFromMinutes(int(break1)+break1.duration, 1)
                out += "\n" + TimetableWithBreaks.align(break1.bt() + "-" + break2.bt()) + "-" * 147
            out += "\n" + "*"*(8*20+7) + "\n"
        return out
