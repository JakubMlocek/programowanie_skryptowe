...
class Break(BasicTerm):
    ...
    @classmethod
    def rebuildFromMinutes(cls, minutes:int, duration):
...
class Term(BasicTerm):
    ...
    @classmethod
    def rebuildFromMinutes(cls, minutes:int):
    ...
    def __sub__(self,term):
        ...
        newTerm = Term.rebuildFromMinutes(int(term))
        intSelf -= intTerm
        if(intTerm < 0):
            intTerm *= -1
    ...
    def minuteDifference(self, term):
        ...
        if(minutes1 < minutes2):
            diff = Term.rebuildFromMinutes(int(self))
            diff.duration = minutes2 - minutes1
        else:
            diff = Term.rebuildFromMinutes(int(term))
            diff.duration = minutes1 - minutes2
        return diff

    def endTime(self):
        return Term.rebuildFromMinutes(int(self)+self.duration)